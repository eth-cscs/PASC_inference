function weak_repeat_and_save(filename_begin,C_small,noise_small,gamma0_small,nmb_of_repeat)
...
% signal with noise
C=repmat(C_small+noise_small,1,nmb_of_repeat);
...
for k = 1:K
   gamma0(1,((k-1)*length(C)+1) : (k*length(C))) = repmat(gamma0_small(1:length(C_small)),1,nmb_of_repeat); 
end
