...
%    my_scale = [0.38 0.46];
    ...
    % x,y
    cap_coordinates_3D(k,:) = [cap_coordinates_2D(k,:), 3];
    direction = projection_center - cap_coordinates_3D(k,:);

    % z
    while norm(cap_coordinates_3D(k,:)./[3.5,2.9,3.0]) > 1 && cap_coordinates_3D(k,3) > -2.0
...
if true
    ...
    % plot 2D
    plot3(cap_coordinates_2D(:,1),cap_coordinates_2D(:,2),3*ones(size(cap_coordinates_2D(:,1))),'r*')
    ...
    for k=1:size(cap_coordinates_2D,1)
        ...
        plot3([cap_coordinates_2D(k,1) projection_center(1)],[cap_coordinates_2D(k,2) projection_center(2)],[3 projection_center(3)],'b-')    
