        ...
        for epssqr in epssqrs:
            for K in Ks:
                for N in Ns:
                    ...
                    problem_parameters_full = "%s --test_image_filename=\"%s\" --test_image_out=\"%s\" --test_width=%s --test_height=%s --test_epssqr=%.10f --test_K=%s --test_shortinfo_header='image_name,width,height,noise,epssqr,K,architecture,N,Nthreads,Ngpu,' --test_shortinfo_values='%s,%d,%d,%s,%f,%d,%s,%d,%d,%d,' --test_shortinfo_filename='shortinfo/%s.txt'" % (problem_parameters, image_path, problem_name_full, dimension[0], dimension[1], epssqr, K, image_name, dimension[0], dimension[1], noise, epssqr, K, architecture, N, Nthreads, Ngpu, problem_name_full);
