/** @file data.h
 *  @brief 
 *
 *  PetscVector specific file for general/data/data.h
 *  
 *  @author Lukas Pospisil
 */

#ifndef PASC_PETSCVECTOR_DATA_H
#define	PASC_PETSCVECTOR_DATA_H

#include "general/data/data.h"

#include "external/petscvector/data/tsdata.h"
//#include "external/petscvector/data/generaldata.h"
//#include "external/petscvector/data/diagdata.h"
#include "external/petscvector/data/edfdata.h"
//#include "external/petscvector/data/entropydata.h"
#include "external/petscvector/data/imagedata.h"
//#include "external/petscvector/data/qpdata.h"
#include "external/petscvector/data/signaldata.h"
//#include "external/petscvector/data/simpledata.h"


#endif
