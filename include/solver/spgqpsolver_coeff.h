...
/* constructor */
SPGQPSolverC_fs::SPGQPSolverC_fs(int new_m){
	this->m = new_m;
	this->fs_list = (double*)malloc(this->m*sizeof(double));
}

SPGQPSolverC_fs::~SPGQPSolverC_fs(){
	free(this->fs_list);
}

/* init the list with function values using one initial fx */
void SPGQPSolverC_fs::init(double fx){
	LOG_FUNC_BEGIN
...
int SPGQPSolverC_fs::get_size(){
	return this->m;
}

/* get the value of max value in the list */
double SPGQPSolverC_fs::get_max(){
	LOG_FUNC_BEGIN
...
void SPGQPSolverC_fs::update(double new_fx){
	LOG_FUNC_BEGIN
...
void SPGQPSolverC_fs::print(ConsoleOutput &output)
{
	output << "[ ";
